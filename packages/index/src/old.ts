export namespace BPlus {
    ...
    export class BPlusTree<K, V> {
        ...
        public find(key: K): V | null {
            ...
            if (found) {
                return leaf.children[index].value as V
            } else {
                return null
            }
        }
        ...
        private split(node: Node<K, V>) {
            ...
            if (!node.isLeaf) {
                let middleNode = (newNode.children.shift() as Child<K, V>).node;
                node.children.push({ key: null, node: middleNode });

                for (let child of newNode.children) {
                    (child.node as Node<K, V>).parent = newNode;
                }
            }

            let parent = node.parent;
            if (parent) {
                let {index} = this.getChildIndex(midKey as K, parent);
        ...
        private findLeaf(key: K, node: Node<K, V>): Node<K, V> {
            if (node.isLeaf) {
                ...
                return this.findLeaf(key, child.node as Node<K, V>);
            }
        }
        ...
        private getChildIndex(key: K, node: Node<K, V>): { index: number, found: boolean } {
            ...
            let comparison = this.compareKey(key, node.children[index].key as K);
            if (comparison == 0) {
        ...
        private getChildIndexBinary(key: K, children: Child<K, V>[], start: number, end: number): number {
            ...
            let comparison = this.compareKey(key, children[mid].key as K);
            if (comparison == 0) {
                return mid;
            } else if (comparison < 0) {
        ...
        private printNode(nodeItem: Child<K, V>, prefix: string, isLast: boolean, isLeaf: boolean, isRoot: boolean) {
            ...
            if (!isLeaf) {
                ...
                for (let i = 0; i < (node as Node<K, V>).children.length; i++) {
                    let isLastChild = (i == (node as Node<K, V>).children.length - 1);
                    this.printNode((node as Node<K, V>).children[i], prefix + (isLast ? "    " : "│   "), isLastChild, ((node as Node<K, V>).isLeaf as boolean), false);
                }
            }
        }
    }
